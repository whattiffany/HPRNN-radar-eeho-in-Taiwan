...
#        print("index=",index,"idx=",idx)
        ...
        for batch in range(idx*self._batch_size, (idx+1)*self._batch_size):
            try:
                ...
                # 原本 batch_X.append(self._getCentral(radar_echo[:self._period], self._input_shape).reshape([self._period]+self._input_shape+[1]))
#                print("np.array(radar_echo[-1]).shape=",np.array(radar_echo[-1]).shape)
                ...
                # 原本 batch_y.append(self._getCentral(radar_echo[-self._predict_period:], self._output_shape).reshape([self._predict_period*self._output_shape[0]*self._output_shape[1]]))  
